...
//we will be giving the bean and it will create the payload for CeateJobAPI 
	...
	public static CreateJobPayload mapper(CreateJobBean bean) {
		...
		CustomerProduct customerProduct=new CustomerProduct(bean.getCustomer_product__dop(),
				...
				bean.getCustomer_product__imei1(), bean.getCustomer_product__imei2(), 
				bean.getCustomer_product__popurl(), oemId, mstWarrantyStatusId);
		...
		CreateJobPayload payload=new CreateJobPayload(mstServiceLocationId,
				mstPlatformId,mstWarrantyStatusId,oemId, customer, customerAddress, customerProduct, problemList);
